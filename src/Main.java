public class Main {
    public static void main(String[] args)
    {
        ...
        if (temperature <= 5) {
            ...
            System.out.println("Сегодня тепло, " + temperature + "можно идти без шапки и в коротких дждинсах");
        }
        ...
        int speed = 90;
        int normalSpeed = 60;
        int deltaSpeed = Math.abs (speed - normalSpeed);
        if (deltaSpeed > 20 && deltaSpeed < 40) {
            int price = 500;
        } else if (deltaSpeed > 40 && deltaSpeed < 80) {
            int price = 2500;
        } else if (deltaSpeed > 80) {
            var price = 5000;
        }
        if (speed > 60) {
            System.out.println("Если твоя скорость - " + speed + "км/ч, придётся заплатить штраф в размере " + price + "рублей");
